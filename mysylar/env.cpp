...
namespace mysylar
{
    ...
    bool Env::init(int argc, char **argv)
    {
        ...
        ssize_t leo_test = readlink(link, path, sizeof(path));
        std::cout << leo_test << std::endl;
        // /path/xxx/exe
        m_exe = path;
        ...
        for (int i = 1; i < argc; ++i)
        {
            if (argv[i][0] == '-')
            {
                ...
                {
                    MYSYLAR_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                                                << " val=" << argv[i];
                    return false;
                }
            }
    ...
    std::string Env::getAbsoluteWorkPath(const std::string &path) const
    {
        ...
        static mysylar::ConfigVar<std::string>::ptr g_server_work_path =
